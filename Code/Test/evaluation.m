...
%模型重建图像路径
dis = dir("D:\韦新杰师兄工作\MWFFNet小修\ablantion_study\levels\L1\*.png");
T = cell(1,length(dis));
P = cell(1,length(dis));
  for i=1:length(dis)
    %模型重建图像路径
    TimageName=strcat("D:\韦新杰师兄工作\MWFFNet小修\ablantion_study\levels\L1\",dis(i).name);
...
%   正常光照图像
    PimageName=strcat('D:\韦新杰师兄工作\Comparsion_method\dataset\LOLV1\eval15\high\',disP,'.png');  %LOLV1正常光照图像
